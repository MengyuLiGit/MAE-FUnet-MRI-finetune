...
from utils.general_dataloader_cache import GeneralDataset, GeneralDatasetMae, PklDatasetWithLabel
...
from sequence_detection.models_unet_finetune import Unet
CHANS = 32
cls_strategy = 'gap' # or 'flatten', 'conv'
model = Unet(
    in_chans=3,
    out_chans=3,
    chans=CHANS,
    if_classify=True,
    class_dim= len(detect_sequence) + 1,
    dim = INPUT_SIZE,
    cls_strategy=cls_strategy
)
print(sum(p.numel() for p in model.parameters()))
print(sum(p.numel() for p in model.parameters() if p.requires_grad))
# in_chans=1,out_chans=1, chans=CHANS, if_classify= True,  class_dim=len(DETECT_SEQUENCE) + 1, dim=RESIZE, mlp_ratio = 32

from sequence_detection.train_unet_finetune import Trainer
optimizer = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=0.05)
loss_fn = torch.nn.CrossEntropyLoss()
TRAIN_EPOCHS = 1000
filtered = False
if filtered:
    path_save = "../saved_models/sequence_detection/max_train_len_unet_" + cls_strategy + "_e" + str(TRAIN_EPOCHS) + '_' + 'filter_slice' + '/'
else:
    path_save = "../saved_models/sequence_detection/max_train_len_unet_" + cls_strategy + "_e" + str(TRAIN_EPOCHS) + '_' + str(max_train_len) + '/'
...
model = trainer.train(epochs=TRAIN_EPOCHS, show_step=500, show_test=True)
