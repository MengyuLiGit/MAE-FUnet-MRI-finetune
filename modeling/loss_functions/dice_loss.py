...
class CombinedDiceFocalCELoss(nn.Module):
    def __init__(self, dice_kwargs={}, focal_kwargs={}, ce_kwargs={}, weights=(1.0, 1.0, 1.0)):
        super().__init__()
        self.dice_loss = SoftDiceLoss(apply_nonlin=softmax_helper, **dice_kwargs)
        self.focal_loss = FocalLoss(apply_nonlin=softmax_helper, **focal_kwargs)
        self.ce_loss = nn.CrossEntropyLoss(**ce_kwargs)
        self.weights = weights  # (w_dice, w_focal, w_ce)

    def forward(self, net_output, target):
        dice = self.dice_loss(net_output, target)
        focal = self.focal_loss(net_output, target)
        ce = self.ce_loss(net_output, target[:, 0].long())
...
class CombinedGDLFocalCELoss(nn.Module):
    def __init__(self, gdl_kwargs={}, focal_kwargs={}, ce_kwargs={}, weights=(1.0, 1.0, 1.0)):
        super().__init__()
        self.gdl_loss = GDL(apply_nonlin=softmax_helper, **gdl_kwargs)
        self.focal_loss = FocalLoss(apply_nonlin=softmax_helper, **focal_kwargs)
        self.ce_loss = nn.CrossEntropyLoss(**ce_kwargs)
        self.weights = weights  # (w_gdl, w_focal, w_ce)

    def forward(self, net_output, target):
        gdl = self.gdl_loss(net_output, target)
        focal = self.focal_loss(net_output, target)
        ce = self.ce_loss(net_output, target[:, 0].long())
