        ...
        for i in pbar:
            ...
            for batch, data_i in enumerate(self.loader_train):
                if self.max_num_batch_train is None or batch < self.max_num_batch_train:
                    images_input = data_i[0].to(self.device)  # [batch, 1, H, W]
                    image_label = data_i[1]
                    image_label = image_label.to(self.device)  # [batch, num_class]
                    patient_label = data_i[2]
                    if i == 0 and batch == 0:
                        print_var_detail(images_input, "images_input")
                        print_var_detail(image_label, "image_label")

                    self.optimizer.zero_grad()
                    image_label_pred = self.model(images_input, task=self.task)
        ...
        with torch.no_grad():
            ...
            for batch, data_i in enumerate(self.loader_test):
                if self.max_num_batch_test is None or batch < self.max_num_batch_test:
                    ...
                    images_input = data_i[0].to(self.device)  # [pair]
                    image_label = data_i[1]
                    image_label = image_label.to(self.device)  # [pair]
                    timer_data.hold()
                    timer_model.tic()
                    image_label_pred = self.model(image=images_input, task=self.task)
                    loss = self.loss(image_label_pred, image_label)
                    if torch.isnan(loss):
                        num_nan += 1
                    else:
    ...
    def train(self, show_step=-1, epochs=5, show_test=True):
        ...
        if self.if_fix_encoder:  # freeze all param in encoder
            ...
            for param in self.model.image_encoder.patch_embed.parameters():
                param.requires_grad = False
            for param in self.model.image_encoder.blocks.parameters():
                param.requires_grad = False
            for param in self.model.image_encoder.norm.parameters():
                param.requires_grad = False
        ...
        for i in pbar:
            ...
            for batch, (images, images_masked, images_mask_binary) in enumerate(self.loader_train):
                if self.max_num_batch_train is None or batch < self.max_num_batch_train:
                    ...
                    self.optimizer.zero_grad()
                    images_mask_binary_pred = self.model(images_input, self.task)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for batch, (images, images_masked, images_mask_binary) in enumerate(self.loader_test):
                if self.max_num_batch_test is None or batch < self.max_num_batch_test:
                    timer_data, timer_model = utility.timer(), utility.timer()
                    images_input = images.to(self.device)  #  [N, 3, H, W]
                    images_masked = images_masked.to(self.device)  #  [N, 1, H, W]
                    images_mask_binary = images_mask_binary.to(self.device) # [N, 1, H, W]
                    timer_data.hold()
                    timer_model.tic()
                    images_mask_binary_pred = self.model(image=images_input, task=self.task) # [N, 1, H, W]
                    ...
                    images_masked_pred = images_input[:, 0, :, :].unsqueeze(1) * images_mask_binary_pred # [N, 1, H, W]
                    loss = self.loss(images_mask_binary_pred, images_mask_binary)
                    if torch.isnan(loss):
                        num_nan += 1
                    else:
...
class TrainerSegBinary:
    ...
    def __init__(self, loader_train, loader_test, my_model, my_loss, optimizer, RESUME_EPOCH,
                 PATH_MODEL, device, max_num_batch_train=None, max_num_batch_test=None,labels=None, if_fix_encoder=False,
                 task='extraction'):
        self.loader_train = loader_train  # list [image, filename, slice_index]
        self.loader_test = loader_test
        self.model = my_model
        ...
        with torch.no_grad():
            ...
            for batch, (images, images_masked, images_mask_binary) in enumerate(self.loader_test):
                if self.max_num_batch_test is None or batch < self.max_num_batch_test:
                    ...
                    images_mask_binary_pred[images_mask_binary_pred > 0.1] = 1
                    images_mask_binary_pred[images_mask_binary_pred <= 0.1] = 0
                    images_masked_pred = images_input[:, 0, :, :].unsqueeze(1) * images_mask_binary_pred  # [N, C, H, W]
                    loss = self.loss(images_mask_binary_pred, images_mask_binary)
                    if torch.isnan(loss):
                        num_nan += 1
                    else:
...
class TrainerSegBinaryDiceLoss:
    ...
    def __init__(self, loader_train, loader_test, my_model, my_loss, optimizer, scheduler, grad_scaler, amp,
                 ...
                 PATH_MODEL, device, max_num_batch_train=None, max_num_batch_test=None, labels=None, if_fix_encoder=False,
                 task='extraction', loss_type = 'dice', use_grad_scaler = False):
        self.loader_train = loader_train  # list [image, filename, slice_index]
        self.loader_test = loader_test
        self.model = my_model
        self.loss = my_loss
            ...
            for batch, (images, images_masked, images_mask_binary) in enumerate(self.loader_train):
                ...
                if self.max_num_batch_train is None or batch < self.max_num_batch_train:
                    images_input = images.to(device=self.device, dtype=torch.float32,
                                             memory_format=torch.channels_last)  # [batch, 3, H, W]
                    # images_masked = images_masked.to(self.device)  # [batch, num_class]
                    ...
                    with torch.autocast(self.device.type if self.device.type != 'mps' else 'cpu', enabled=self.amp):
                        images_mask_binary_pred = self.model(images_input, self.task)  # [batch, n_classes+1, H, W]
                        if self.n_classes == 1:
                            ...
                            if self.loss_type == 'dice':
                                ...
                                loss = self.loss(images_mask_binary_pred,
                                                 images_mask_binary)  # define self.loss as dice_focal loss externally
                            else:
                                loss = self.loss(images_mask_binary_pred,
                                                 images_mask_binary)
                        self.optimizer.zero_grad()
                        if torch.isnan(loss):
                            ...
                            # print(filename)
                            print('images_mask_binary_pred', images_mask_binary_pred)
                            print('images_mask_binary', images_mask_binary)
                            for j in range(images_input.size(0)):
                                print_var_detail(images_input[j], 'images_input')
                            num_nan += 1
                        else:
            ...
            with torch.autocast(self.device.type if self.device.type != 'mps' else 'cpu', enabled=self.amp):
                for batch, (images, _, images_mask_binary) in enumerate(self.loader_test):
                    if self.max_num_batch_test is None or batch < self.max_num_batch_test:
                        ...
                        # evaluation metrics
                        images_input = images_input[:,0,:,:].unsqueeze(1) #[batch, 1, H, W]
                        if self.n_classes == 1:
...
class TrainerMaeUnetSegBinaryDiceLoss:
    """
            """
    def __init__(self, loader_train, loader_test, my_model, my_loss, optimizer, scheduler, grad_scaler, amp,
                 gradient_clipping, RESUME_EPOCH,
                 PATH_MODEL, device, max_num_batch_train=None, max_num_batch_test=None, labels=None, if_fix_encoder=False,
                 loss_type = 'dice', use_grad_scaler = False):
        self.loader_train = loader_train  # list [image, filename, slice_index]
        self.loader_test = loader_test
        self.model = my_model
        self.loss_conv = my_loss[0]
        self.loss_tran = my_loss[1]
        self.optimizer = optimizer
        self.PATH_MODEL = PATH_MODEL
        self.RESUME_EPOCH = RESUME_EPOCH
        self.cpu = False
    ...
    def train(self, show_step=-1, epochs=5, show_test=True):
        ...
        optimizer_to(self.optimizer, self.device)
        self.model.train()
        self.loader_train.is_train = True
        ...
        for i in pbar:
            ...
            for batch, (images, images_masked, images_mask_binary) in enumerate(self.loader_train):
                ...
                if self.max_num_batch_train is None or batch < self.max_num_batch_train:
                    ...
                    # cause nan error, uncomment for now
                    # with torch.autocast(self.device.type if self.device.type != 'mps' else 'cpu', enabled=self.amp):
                    if True:
                        images_mask_binary_pred_conv, images_mask_binary_pred_tran = self.model(images_input)  # [batch, n_classes+1, H, W]
                        if self.n_classes == 1:
                            loss_conv = self.loss_conv(images_mask_binary_pred_conv.squeeze(1), images_mask_binary.float())
                            loss_conv += dice_loss(F.sigmoid(images_mask_binary_pred_conv.squeeze(1)), images_mask_binary.float(),
                                              multiclass=False)
                            loss_tran = self.loss_tran(images_mask_binary_pred_tran.squeeze(1),
                                                       images_mask_binary.float())
                            loss_tran += dice_loss(F.sigmoid(images_mask_binary_pred_tran.squeeze(1)),
                                                   images_mask_binary.float(),
                                                   multiclass=False)
                        else:
                            if self.loss_type == 'dice':
                                loss_conv = self.loss_conv(images_mask_binary_pred_conv, images_mask_binary)
                                loss_conv += dice_loss(
                                    F.softmax(images_mask_binary_pred_conv, dim=1).float(),
                                    F.one_hot(images_mask_binary, self.n_classes + 1).permute(0, 3, 1, 2).float(),
                                    multiclass=True
                                )
                                loss_tran = self.loss_tran(images_mask_binary_pred_tran, images_mask_binary)
                                loss_tran += dice_loss(
                                    F.softmax(images_mask_binary_pred_tran, dim=1).float(),
                                    F.one_hot(images_mask_binary, self.n_classes + 1).permute(0, 3, 1, 2).float(),
                                    multiclass=True
                                )
                            elif self.loss_type == 'dice_focal':
                                loss_conv = self.loss_conv(images_mask_binary_pred_conv,
                                                 images_mask_binary)  # define self.loss as dice_focal loss externally
                                loss_tran = self.loss_tran(images_mask_binary_pred_tran,
                                                           images_mask_binary)
                            else:
                                loss_conv = self.loss_conv(images_mask_binary_pred_conv,
                                                           images_mask_binary)
                                loss_tran = self.loss_tran(images_mask_binary_pred_tran,
                                                 images_mask_binary)
                        loss = loss_conv + loss_tran # combine loss
                        self.optimizer.zero_grad()
                        if torch.isnan(loss):
                            print(batch)
                            print("nan loss occur")
                            # print(filename)
                            print_var_detail(images_mask_binary_pred_conv, 'images_mask_binary_pred_conv')
                            print_var_detail(images_mask_binary_pred_tran, 'images_mask_binary_pred_tran')
                            print_var_detail(images_mask_binary, 'images_mask_binary')
                            print_var_detail(images_input, 'images_input')
                            # for j in range(images_input.size(0)):
                            #     print_var_detail(images_input[j], 'images_input')
    ...
    def test(self):
        ...
        Test the reconstruction performance. WNet.
        '''
        self.loader_test.is_train = False
        self.model = self.model.to(self.device)
        self.model.eval()
        ...
        with torch.no_grad():
            ...
            # with torch.autocast(self.device.type if self.device.type != 'mps' else 'cpu', enabled=self.amp):
            if True:
                print('total test batch: ', len(self.loader_test))
                for batch, (images, _, images_mask_binary) in enumerate(self.loader_test):
                    if self.max_num_batch_test is None or batch < self.max_num_batch_test:
                        timer_data, timer_model = utility.timer(), utility.timer()
                        images_input = images.to(device=self.device, dtype=torch.float32,
                        ...
                        # loss = self.loss(images_mask_binary_pred, images_mask_binary)
                        if self.n_classes == 1:
                            loss_conv = self.loss_conv(images_mask_binary_pred_conv.squeeze(1),
                                                       images_mask_binary.float())
                            loss_conv += dice_loss(F.sigmoid(images_mask_binary_pred_conv.squeeze(1)),
                                                   images_mask_binary.float(),
                                                   multiclass=False)
                            loss_tran = self.loss_tran(images_mask_binary_pred_tran.squeeze(1),
                                                       images_mask_binary.float())
                        ...
                        # max
                        images_mask_binary_pred = torch.max(images_mask_binary_pred_conv, images_mask_binary_pred_tran)
