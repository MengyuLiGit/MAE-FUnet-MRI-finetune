...
@register_model
def mae_vit_base_patch16_test(pretrained=None, **kwargs):
    model = MAE_VisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), num_classes=1000, drop_path_rate=0.1,
        global_pool=True, **kwargs)
    ...
    if pretrained is not None:
        ...
        # )
        # model.load_state_dict(checkpoint["model"], strict=False)
        print("load mae_vit_base_patch16")
        model = load_pretrain_model(model, model_path=os.path.expanduser(pretrained))
    return model
@register_model
...
def Conformer_small_patch16_test(pretrained=None, **kwargs):
    model = Conformer(patch_size=16, channel_ratio=4, embed_dim=384, depth=12,
                      num_heads=6, mlp_ratio=4, qkv_bias=True, **kwargs)
    if pretrained is not None:
        model = load_pretrain_model(model, model_path=os.path.expanduser(pretrained))
...
@register_model
def Transconv_small_patch16_test(pretrained_vit=None, pretrained_conformer=None, **kwargs):
    if pretrained_vit and pretrained_conformer:
        print("load both vit and conformer")
        pre_trained_vit = mae_vit_base_patch16_test(pretrained=pretrained_vit)
        # pre_trained_vit = None  # uncomment to initialize only conv tower
        pre_trained_conformer = Conformer_small_patch16_test(pretrained=pretrained_vit)
        model = TransConv(patch_size=16, channel_ratio=4, embed_dim=768 if pre_trained_vit is not None else 384, depth=12,
                        num_heads=6, mlp_ratio=4, qkv_bias=True,
                        pre_trained_vit=pre_trained_vit, vit_depth=12, pre_trained_conformer=pre_trained_conformer,
                        additive_fusion_down=False, additive_fusion_up=False, up_ftr_map_size=[56]+[56]*3+[28]*4+[14]*4, down_ftr_map_size=[197]*12,
        ...
        pre_trained_vit = mae_vit_base_patch16_test(pretrained=pretrained_vit)
        # pre_trained_vit = None  # uncomment to initialize only conv tower
        model = TransConv(patch_size=16, channel_ratio=4, embed_dim=768 if pre_trained_vit is not None else 384,
                          depth=12,
                          num_heads=6, mlp_ratio=4, qkv_bias=True,
                          pre_trained_vit=pre_trained_vit, vit_depth=12,
                          additive_fusion_down=False, additive_fusion_up=False,
                          up_ftr_map_size=[56] + [56] * 3 + [28] * 4 + [14] * 4, down_ftr_map_size=[197] * 12,
                          **kwargs)
    elif pretrained_conformer:
        print("load conformer")
        # pre_trained_vit = None  # uncomment to initialize only conv tower
        pre_trained_conformer = Conformer_small_patch16_test(pretrained=pretrained_conformer)
        model = TransConv(patch_size=16, channel_ratio=4, embed_dim=768 if pretrained_vit is not None else 384, depth=12,
                        num_heads=6, mlp_ratio=4, qkv_bias=True,
                        vit_depth=12, pre_trained_conformer=pre_trained_conformer,
                        additive_fusion_down=False, additive_fusion_up=False, up_ftr_map_size=[56]+[56]*3+[28]*4+[14]*4, down_ftr_map_size=[197]*12,
                        **kwargs)
    else:
        model = TransConv(patch_size=16, channel_ratio=4, embed_dim=768, depth=12,
...
@register_model
def Transconv_small_patch16_Raw(pretrained_vit=None, pretrained_conformer=None, **kwargs):
    if pretrained_vit and pretrained_conformer:
        print("load both vit and conformer")
        pre_trained_vit = mae_vit_base_patch16_test(pretrained=pretrained_vit)
        # pre_trained_vit = None  # uncomment to initialize only conv tower
