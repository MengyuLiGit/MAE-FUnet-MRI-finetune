...
from modeling.unet import ConvBlock as ConvBlockUnet, TransposeConvBlock
...
class SegmentationDecoder(nn.Module):
    def __init__(self, hidden_dim, num_classes, img_size, patch_size):
        ...
        self.patch_dim = img_size // patch_size  # H_patch = W_patch
        self.num_upsample_layers = int(math.log2(img_size // self.patch_dim))  # Number of upsampling layers

        # Create upsampling layers dynamically with LayerNorm
        layers = []
        in_channels = hidden_dim
        for _ in range(self.num_upsample_layers - 1):  # Exclude the final output layer
            layers.append(nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2))
            layers.append(nn.LayerNorm([in_channels // 2, self.patch_dim * 2, self.patch_dim * 2]))
            layers.append(nn.ReLU())
            in_channels //= 2
            self.patch_dim *= 2

        # Final layer to match the number of classes
        layers.append(nn.ConvTranspose2d(in_channels, num_classes, kernel_size=2, stride=2))
        self.decoder = nn.Sequential(*layers)

    def forward(self, x):
        # Input x is [B, hidden_dim, H_patch, W_patch]
        return self.decoder(x)  # Output is [B, num_classes, H_img, W_img]
...
class ConvTransBlock(nn.Module):
    ...
    def forward(self, x, x_t):
        ...
        elif self.has_pre_conv_block: # allow expand_block and fusion_block to be finetuned I guess
...
class TransConv(nn.Module):
    ...
    def forward(self, x, monitor=False, writer=None, global_step=None):
        ...
        print('1x ', x.shape)
        print('1x_t ', x_t.shape)

        # 2 ~ final 
        for i in range(2, self.fin_stage):         
            x, x_t = eval('self.conv_trans_' + str(i))(x, x_t)
            print(str(i), 'x ', x.shape)
            print(str(i), 'x_t ', x_t.shape)
            if monitor:
        ...
        for i in range(self.fin_stage, self.vit_final_stage):         
            ...
            print(str(i), 'finalx_t ', x_t.shape)
...
class TransConvRaw(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, base_channel=64, channel_ratio=4, num_med_block=0,
        ...
        # Stem stage: get the feature maps by conv block (copied form resnet.py)
        # self.conv1 = nn.Conv2d(in_chans, base_channel, kernel_size=7, stride=2, padding=3, bias=False) if pre_trained_conformer is None else pre_trained_conformer.conv1  # 1 / 2 [112, 112]


        # self.conv1 = nn.Conv2d(in_chans, base_channel, kernel_size=3, stride=1, padding=1,
        #                        bias=False) if pre_trained_conformer is None else pre_trained_conformer.conv1  # 1 / 2 [112, 112]

        # self.conv1 = ConvBlock(inplanes=in_chans, outplanes=base_channel, res_conv=True, stride=1)
        # self.bn1 = nn.BatchNorm2d(base_channel) if pre_trained_conformer is None else pre_trained_conformer.bn1
        # self.act1 = nn.ReLU(inplace=True)
        # self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  # 1 / 4 [56, 56]

        # using unet head
        self.conv1 = ConvBlockUnet(in_chans=in_chans, out_chans= base_channel, drop_prob=0.0, use_res=True) if pre_trained_conformer is None else pre_trained_conformer.conv1 # [256, 256]
        # output = F.avg_pool2d(output, kernel_size=2, stride=2, padding=0)
        self.conv2 = ConvBlockUnet(in_chans=base_channel, out_chans=base_channel * 2, drop_prob=0.0, use_res=True) if pre_trained_conformer is None else pre_trained_conformer.conv2 # [112, 112]
        # output = F.avg_pool2d(output, kernel_size=2, stride=2, padding=0)



        # using self defined decoder pred
        ...
        stage_1_channel = int(base_channel * channel_ratio)
        trans_dw_stride = patch_size // 4
        self.conv_1 = ConvBlock(inplanes=base_channel * 2, outplanes=stage_1_channel, res_conv=True, stride=1) if pre_trained_conformer is None else pre_trained_conformer.conv_1
        if pre_trained_vit is not None:
            # self.pos_drop = pre_trained_vit.pos_drop
            ...
            self.trans_patch_conv = nn.Conv2d(base_channel, embed_dim, kernel_size=trans_dw_stride, stride=trans_dw_stride, padding=0) if pre_trained_conformer is None else pre_trained_conformer.trans_patch_conv
            self.trans_1 = TransformerBlock(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                                qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=self.trans_dpr[0],
                                ) if pre_trained_conformer is None else pre_trained_conformer.trans_1
        ...
        if pre_trained_conformer is None:
            ...
            for _ in range(num_pool_layers):
                ...
                self.up_conv.append(ConvBlockUnet(ch * 2, ch, 0.0, True))
                ch //= 2
            self.out_conv = nn.Conv2d(ch * 2, self.out_chans, kernel_size=1, stride=1)
        else:
            self.up_conv = pre_trained_conformer.up_conv
            self.up_transpose_conv = pre_trained_conformer.up_transpose_conv
            self.out_conv = pre_trained_conformer.out_conv
    ...
    def _init_weights(self, m):
        # don't initialize weight if it is initialized by pretrained model!
        if is_pretrain(m):
            print('model has pretrained weights')
            return
        else:
            print('model does not have pretrained weights, initialize weights')
    ...
    def forward(self, x, monitor=False, writer=None, global_step=None):
        ...
        # stem stage [N, 3, 224, 224] -> [N, 64, 56, 56]
        # print('0x ', x.shape)
        # x_base = self.act1(self.bn1(self.conv1(x))) # before max pool
        # print('0x before max pool ', x_base.shape)
        #
        # x_base = self.maxpool(x_base)
        # print('0x ', x_base.shape)

        stack = []

        # unet head
        x_base = self.conv1(x)
        # print('0x unet0', x_base.shape)
        stack.append(x_base)
        x_base = F.avg_pool2d(x_base, kernel_size=2, stride=2, padding=0)
        x_base = self.conv2(x_base)
        # print('0x unet1', x_base.shape)
        stack.append(x_base)
        x_base = F.avg_pool2d(x_base, kernel_size=2, stride=2, padding=0)
        # print('0x unet output', x_base.shape)
        ...
        ## tran decoder
        x_t = self.pre_trained_vit.decoder_embed(x_t)
        ...
        # add pos embed
        x_t = x_t + self.pre_trained_vit.decoder_pos_embed

        # apply Transformer blocks
        for blk in self.pre_trained_vit.decoder_blocks:
            x_t = blk(x_t)
        x_t = self.pre_trained_vit.decoder_norm(x_t)

        # predictor projection
        x_t = self.decoder_pred(x_t)

        # remove cls token
        x_t = x_t[:, 1:, :]
        x_t = self.pre_trained_vit.unpatchify(x_t, out_chans=self.out_chans)

        # add unet skip connection
        stack_unet = stack[:2]
        step = self.depth // 3
        for i in range(2, len(stack), step):
            end = min(i + step, len(stack))
            stacked_tensor = torch.stack(stack[i:end])
            stack_unet.append(torch.mean(stacked_tensor, dim=0))

        output = stack_unet[-1]
        stack_unet = stack_unet[:-1]
        # apply up-sampling layers
        for transpose_conv, conv in zip(self.up_transpose_conv, self.up_conv):
            downsample_layer = stack_unet.pop()
        ...
        return [output, x_t] # conv_out, tran_out


class TransConvRawSeg(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, base_channel=64, channel_ratio=4, num_med_block=0,
                 ...
                 additive_fusion_down=False, additive_fusion_up=False, up_ftr_map_size=None, down_ftr_map_size=None,
                 qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0., decoder_embed_dim=512, out_chans = 7
                 , img_size = 256,global_pool = False):
        ...
        # using self defined decoder pred
        # self.decoder_pred = nn.Linear(decoder_embed_dim, patch_size ** 2 * out_chans, bias=True)
        self.decoder = SegmentationDecoder(hidden_dim=embed_dim, num_classes=out_chans, img_size=img_size,
    ...
    def forward(self, x, monitor=False, writer=None, global_step=None):
        ...
        # remove cls token
        x_t = x_t[:, 1:, :]
        batch_size, num_patches, hidden_dim = x_t.size()
        height = width = int(math.sqrt(num_patches))  # Calculate spatial dimensions
        features = x_t.permute(0, 2, 1).reshape(batch_size, hidden_dim, height, width)

        # Pass through the decoder to get segmentation map
        x_t = self.decoder(features)
...
class TransConvRawClassify(nn.Module):
    def __init__(self, patch_size=16, in_chans=3, base_channel=64, channel_ratio=4, num_med_block=0,
                 embed_dim=768, depth=12, num_heads=12, mlp_ratio=4., qkv_bias=False,
                 pre_trained_vit=None, finetune_vit=False, vit_depth=12, pre_trained_conformer=None, finetune_conv=True,
                 additive_fusion_down=False, additive_fusion_up=False, up_ftr_map_size=None, down_ftr_map_size=None,
                 qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0., decoder_embed_dim=512, out_chans = 7
                 , img_size = 256,global_pool = False, num_class = 1):
        ...
        self.decoder_pred = Mlp(in_features=embed_dim, hidden_features=embed_dim * 2, out_features=self.num_class)
    ...
    def forward(self, x, monitor=False, writer=None, global_step=None):
        ...
        for i in range(2, self.fin_stage):
            ...
            if monitor:
                log_ftr_map_histograms(writer, x, x_t, i, global_step)
                feature_maps[str(i) + 'x'] = x
                feature_maps[str(i) + 'x_t'] = x_t
        ...
        if monitor:
            return [output, x_t, feature_maps]
