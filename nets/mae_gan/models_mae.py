...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        # x [N,L_masked(H/patch_size * W/patch_size * (1-mask_ratio)) +1(channel 0 for cls_token),D (patch_size**2*3)]
        # mask [N,L(H/patch_size * W/patch_size)]

        return x, mask, ids_restore

    def discriminator(self, currupt_img):
        # currupt_img = currupt_img.clone().detach()  # detach input from mae encoder-decoder
        ...
        # x [N,L(H/patch_size * W/patch_size) + 1(channel 0 for cls_token),1]
        return torch.sigmoid(x)

    def discriminator_loss(self, x, mask):
        x_in = x.clone().detach()  # detach input from mae encoder-decoder generator
    ...
    #     target = 1 - mask  # This flips the mask values, 0 is the removed and reconstructed, 1 is kept / viewable, [N,L]
    #     output = self.discriminator(currupt_img)
    #     disc_preds = output[:, 1:, 0]  # [N,L]
    #
    #     # Reshape target to match the discriminator output shape
    #     # target = target.view(disc_preds.shape)  # no use
    #     target = target.float()
    #
    ...
    def adv_loss(self, currupt_img, mask):
        ...
        # Calculate the number of correct predictions for reconstructed patches
        # corr_recons = (torch.log((1 - disc_preds + 1e-8)) * (1 - target)).sum() / ((1 - target).sum())
        corr_recons = - (torch.log(disc_preds + 1e-8).sum()) / ((1 - target).sum())
        return corr_recons
    ...
    def forward(self, imgs, mask_ratio=0.75):
        ...
        mask1 = mask.unsqueeze(-1).expand_as(pred)  # [N,L,1], 0 is keep, 1 is remove
