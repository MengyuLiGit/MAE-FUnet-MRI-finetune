...
def aw_loss(L_mae, L_adv, Gen_opt, Gen_net):
    ...
    # grad_real_nm_set = set([name for name, param in Gen_net.named_parameters() if param.grad is not None])
    # grad_real_nm = [name for name, param in Gen_net.named_parameters() if param.grad is not None]
    ...
    # tensor with real gradients
    # grad_fake_nm_set = set([name for name, param in Gen_net.named_parameters() if param.grad is not None])
    # grad_fake_nm = [name for name, param in Gen_net.named_parameters() if param.grad is not None]
    ...
    # grad_final_nm_set = set([name for name, param in Gen_net.named_parameters() if param.grad is not None])
    # print('diff fake-final', grad_fake_nm_set.difference(grad_final_nm_set))
    # print('diff fake-real', grad_fake_nm_set.difference(grad_real_nm_set))
    # for index, (_, param) in enumerate(Gen_net.named_parameters()):
    #     # print('index', index)
    #     # print(grad_real_tensor[index].shape)
    #     # print(grad_fake_tensor[index].shape)
